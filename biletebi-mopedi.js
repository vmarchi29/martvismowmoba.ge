...
function insertAds() {
  ...
  // წაშალე ძველი დინამიური რეკლამები
  document.querySelectorAll(".dynamic-ad").forEach(el => el.remove());

  const visible = Array.from(document.querySelectorAll("[class*='question-container'].show"));
  if (visible.length === 0) return;

  function createAd(className) {
    const ad = document.createElement("div");
    ad.className = `${className} dynamic-ad`;
    ad.innerHTML = `<p>თქვენი რეკლამის ადგილი</p>`;
    return ad;
  }

  // მე-5 კითხვის შემდეგ — ads2
  if (visible[4]) {
    visible[4].insertAdjacentElement("afterend", createAd("ads2"));
  }

  // მე-10 კითხვის შემდეგ — ads3
  if (visible[9]) {
    visible[9].insertAdjacentElement("afterend", createAd("ads3"));
  }

  // ბოლო კითხვის შემდეგ — ads4
